...
import (
	...
	_ "github.com/hex0wn/godnslog/server/docs" // docs is generated by Swag CLI, you have to import it.
...
func (self *WebServer) Run() error {
	...
	// try to bind host
	log := gin.Default()
	log.Any("/*any", self.record)

	cfg := self.WebServerConfig
	webDomain := cfg.WebDomain + "." + cfg.Domain
	hs := make(HostSwitch)
	hs[webDomain] = r
	hs["default"] = log
	//http.ListenAndServe(self.Listen, hs)
	...
	self.ss = ss
	go func(){
		ss.ListenAndServeTLS("server.crt", "server.key")
	}()
	return s.Serve(l)
}
